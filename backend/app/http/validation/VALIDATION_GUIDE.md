...
The following fields are **read-only** and automatically generated by the backend:

- `id` - Auto-numbered on creation
- `created_at` - Timestamp of when the record was created (generated server-side)

**Do not include these fields in request payloads** - they will be ignored during load/validation.

### Transaction Date Fields

Numo distinguishes between:

- **Transaction date** (`expensed_at` for debits, `credited_at` for credits) - When the transaction actually occurred (user-provided, required)
- **Creation timestamp** (`created_at`) - When the transaction record was created in the system (auto-generated, read-only)

This separation allows users to record past or future transactions while maintaining an audit trail of when they were entered.
...
def create_debit():
    try:
        ...
        # Expected fields: category_id, place_id, amount, expensed_at (required)
        #                 concept, method, observations (optional)
        # Note: Do not send 'created_at' - it's auto-generated by the server
        validated_data = schema.load(request.get_json())
    except ValidationError as err:
        return jsonify({'errors': err.messages}), 400
    ...
    try:
        ...
        # Create debit with validated data (including expensed_at)
